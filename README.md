...
| 卒業提出（2026年8月3日予定） | 🔲 進行中（結合テスト・セキュリティレビュー・ドキュメント整備・ライセンス選定） |
...
- iMacからのエクスポート手順書の整備（コマンド実行での案内。GUI化は見送り）
- Windows側はViewerアプリのインストール＋アーカイブフォルダの手動パス指定で確認できる状態（実装済み。案内文書のみ整備）
- ここまでの実装分に対する結合テスト・セキュリティレビュー
- ここまでの内容に対するドキュメント整備
- ライセンス選定（2026年8月1日目処で結論）
